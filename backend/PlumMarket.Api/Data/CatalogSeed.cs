...
/// <summary>Demo catalog (categories, products, stock, discounts, reviews) and chat inbox.</summary>
public static class CatalogSeed
{
    ...
    public static List<Product> SeedCatalog(AppDbContext db, Random rnd, List<Branch> branches, DateTime now)
    {
        ...
        for (var i = 0; i < Products.Length; i++)
        {
            ...
            // About two thirds already have ИКПУ filled in; the rest are left for "Сгенерировать ИКПУ".
            if (i % 3 != 2 && IkpuCatalog.Suggest(p.Ru, cat.Name.Get()) is { } ikpu)
            {
                product.Ikpu = ikpu.Code;
                product.PackageCode = ikpu.PackageCode;
                product.UnitCode = ikpu.UnitCode;
            }
    ...
    static void SeedChats(AppDbContext db, Random rnd, List<Product> products, List<Customer> customers, List<Review> reviews, DateTime now)
    {
        ...
        for (var i = 0; i < chatCustomers.Count; i++)
        {
            ...
            {
                Customer = c, Channel = channel, DisplayName = c.FullName,
                Handle = channel == ChatChannel.Instagram ? "@" + (c.Username ?? "guest") : c.Username is null ? null : "@" + c.Username,
            };
        ...
        // Wolt threads come from the aggregator, not from a known customer.
        for (var i = 0; i < 3; i++)
        {
            var t = now.AddHours(-rnd.Next(2, 72));
            var conv = new Conversation { Channel = ChatChannel.Wolt, DisplayName = $"Wolt · заказ W-{rnd.Next(10000, 99999)}" };
            conv.Messages.Add(new ChatMessage { Direction = MessageDirection.In, Text = "Курьер Wolt на месте, заказ готов?", SentAt = t });
            conv.Messages.Add(new ChatMessage { Direction = MessageDirection.Out, Text = "Да, выдаём через 2 минуты.", SentAt = t.AddMinutes(1), SenderName = "Азиз Каримов" });
            conversations.Add(conv);
        }
        ...
        {
            [nameof(ChatChannel.Telegram)] = "Здравствуйте! Мы получили ваше сообщение и ответим в течение 10 минут.",
            [nameof(ChatChannel.Instagram)] = "Спасибо, что написали нам в Instagram! Меню и заказ — в нашем Telegram-боте @plum_bakery_bot.",
            [nameof(ChatChannel.Website)] = "",
            [nameof(ChatChannel.Wolt)] = "",
        };
        db.SaveChanges();
    }
}
