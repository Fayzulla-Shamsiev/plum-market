...
import { api, type CustomerRow, type Paged } from '../api'
...
async function load() {
  error.value = ''
  try {
    data.value = await api.customers({ search: search.value.trim(), sort: sort.value, page: page.value })
  } catch (e) {
  ...
  <div class="page">
    ...
    <div class="card">
      <div class="toolbar">
        <label class="search">
          <Icon name="search" />
          <input v-model="search" class="input" placeholder="Имя, телефон или e-mail" aria-label="Поиск клиентов" />
        </label>
      ...
      <div class="table-wrap">
        <table class="data">
          <thead>
            <tr>
              <th>Имя клиента</th><th>Телефон</th><th>E-mail</th><th class="right">Заказов</th>
              <th class="right">Бонусы</th><th>Добавлен</th><th>Последний визит</th><th class="right">Действие</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr v-if="data && !data.items.length"><td colspan="8" class="empty">Клиентов не найдено</td></tr>
